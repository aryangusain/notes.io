...
const Navbar = () => {
    ...
    const toggleTab = useProfileStore((state) => state.toggleState);
    const triggerRef = useRef<HTMLImageElement>(null);
    const profileRef = useOutsideClick(() => setTabOpen(false), [triggerRef]);
    const {theme, setTheme} = useTheme();


    return (
        <div className="flex items-center md:w-[600px] w-full justify-between md:py-[10px] px-[10px] py-[6px] bg-transparent">
            <IconLayoutSidebar onClick={() => setSidebarOpen(!sidebarOpen)} className="h-[28px] w-[28px] md:absolute left-[20px] cursor-pointer"/>
            ...
            <div className="flex items-center justify-center relative">
                ...
                { session?.user?.image && 
                    <img src={session?.user?.image} ref={triggerRef} onClick={toggleTab} alt="user image" className="select-none cursor-pointer rounded-full h-[32px] w-[32px]"/>
                }
                {
                    tabOpen &&
                    <div ref={profileRef} className="dark:bg-[#27272a] bg-neutral-200 z-30 absolute top-10 -right-2 flex flex-col gap-[16px] py-[20px] px-[60px] shadow-xl rounded-xl items-center">
